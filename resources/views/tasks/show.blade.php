            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg sm:rounded-lg">
                <div class="p-6">
                   ...
                   {{-- Form Tambah Tugas (Bisa disembunyikan) --}}
                    ...
                    {{-- =================================================================== --}}
                    <div class="border border-gray-200 dark:border-gray-700 sm:rounded-lg overflow-hidden">
                        {{-- Tambahkan pembungkus dengan tinggi tetap di sini --}}
                        <div class="overflow-y-auto overflow-x-auto h-[500px] scrollbar-thin scrollbar-thumb-gray-400">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-700 sticky top-0 z-10"> {{-- Sticky supaya header tidak ikut tergulung --}}
                                    <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="relative inline-block text-left">
                                                    ...
                                                    <div x-show="actionDropdownOpen"
                                                        ...
                                                        x-transition
                                                        class="absolute right-0 z-50 w-56 rounded-md shadow-lg bg-white dark:bg-gray-700 ring-1 ring-black ring-opacity-5 focus:outline-none 
                                                        {{ ($loop->last && $loop->count > 1) ? 'bottom-full mb-2 origin-bottom-right' : 'mt-2 origin-top-right' }}"
                                                        ...
                                                        <div class="py-1" role="none">
                                                            ...
                                                            @if(auth()->user()->role === 'staff')
                                                                ...
                                                                {{-- 1. Jika tugas belum diklaim --}}
                                                                @if(is_null($dailyTask->assigned_to_staff_id))
                                                                    <form action="{{ route('dailytasks.take', $dailyTask->id) }}" method="POST">
                                                                        @csrf
                                                                        @method('PATCH')
                                                                        <button type="submit" class="w-full text-left text-sm px-4 py-2 text-green-600 hover:bg-gray-100 dark:hover:bg-gray-600 font-semibold">
                                                                            Ambil Tugas
                                                                        </button>
                                                                    </form>

                                                                {{-- 2. Jika status revisi --}}
                                                                ...
                                                                {{-- 4. Jika sudah diklaim oleh staff yang bersangkutan --}}
                                                                @elseif($dailyTask->assigned_to_staff_id === auth()->id())
                                                                    <a href="{{ route('dailytasks.upload.form', $dailyTask->id) }}"
                                                                        class="text-blue-600 dark:text-blue-400 block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600">
                                                                        Upload Pekerjaan
                                                ...
                                                <div x-show="showEditModal"
                                                    ...
                                                    <div @click.away="showEditModal = false"
                                                        class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-8 w-full max-w-2xl transform transition-all">
                                                        <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4 border-b pb-2 dark:border-gray-700">
                                                            Edit Tugas Harian
                                                        </h3>
                                                        ...
                                                        <form action="{{ route('dailytasks.update', $dailyTask->id) }}" method="POST">
                                                            ...
                                                            <div class="space-y-4">
                                                                {{-- ITEM PEKERJAAN - Full Width --}}
                                                                <div>
                                                                    @if($showTemplate)
                                                                        <x-input-label for="edit_name_{{ $dailyTask->id }}" value="Nama Tugas Harian" />
                                                                        <select id="edit_name_{{ $dailyTask->id }}" name="name"
                                                                            class="block mt-1 w-full text-sm dark:bg-gray-900 dark:text-gray-200 border-gray-300 dark:border-gray-700 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                                                        ...
                                                                        <div class="mt-2">
                                                                            <x-input-label for="edit_manual_name_{{ $dailyTask->id }}" value="Nama Manual (Jika tidak ada di template)" class="text-xs" />
                                                                            <x-text-input id="edit_manual_name_{{ $dailyTask->id }}" 
                                                                                class="block mt-1 w-full dark:bg-gray-900 text-sm" 
                                                                                type="text" name="manual_name"
                                                                    ...
                                                                    @else
                                                                        ...
                                                                        <select name="project_item_id" required
                                                                            class="block mt-1 w-full text-sm dark:bg-gray-900 dark:text-gray-200 border-gray-300 dark:border-gray-700 rounded-md shadow-sm">
                                                                            @foreach($task->project->checklists as $checklist)
                                                                ...
                                                                <div class="grid grid-cols-2 gap-4">
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <input type="date" name="due_date"
                                                                            ...
                                                                            class="block mt-1 w-full text-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 rounded-md shadow-sm dark:[color-scheme:dark]"
                                                                            required>
                                                                    </div>
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <input type="number" name="weight"
                                                                            ...
                                                                            min="1" max="10"
                                                                            class="block mt-1 w-full text-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 rounded-md shadow-sm"
                                                                            required>
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <textarea name="description"
                                                                        class="block mt-1 w-full text-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 rounded-md shadow-sm"
                                                                        rows="2">{{ $dailyTask->description }}</textarea>
                                                                </div>

                                                                {{-- ASSIGN --}}
                                                                <div>
                                                                    <x-input-label value="Tugaskan ke" />
                                                            ...
                                                            <div class="mt-6 flex justify-end space-x-2 pt-4 border-t dark:border-gray-700">
                                                                <button type="button" @click="showEditModal = false"
                                                                    class="px-4 py-2 text-sm bg-gray-200 dark:bg-gray-700 dark:text-gray-200 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600">
                                                                    Batal
                                                                </button>
                                                                <button type="submit"
                                                                    class="px-4 py-2 text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700 shadow-md">
                                                                    Simpan Perubahan
                                                                </button>
                                                            </div>
                                                        </form>
